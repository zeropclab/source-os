...
from packages.adapters.source_probe import FixtureSourceProbeAdapter
from tests.factories import create_source


async def _create_config(client, db):
...
async def test_operator_can_run_and_retrieve_a_bounded_probe(client, db):
    ...
    app.dependency_overrides[get_source_probe_adapter] = lambda: FixtureSourceProbeAdapter(
        scenario="accessible_with_context"
    )
...
async def test_rate_limit_is_reported_with_unverified_context_capabilities(client, db):
    ...
    assert body["sample_available"] is False
    assert body["pagination_supported"] is False
    assert body["replies_supported"] is False
    assert body["context_risks"] == [
...
async def test_runtime_stops_a_probe_at_its_time_budget_and_records_failure(client, db):
    class HangingAdapter:
        async def probe(self, config, *, budget):
            await asyncio.sleep(budget.time_limit_seconds + 1)
